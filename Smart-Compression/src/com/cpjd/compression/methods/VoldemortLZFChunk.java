 ...
 * 
 */
public class VoldemortLZFChunk {
    ...
    final byte[] _data;
    VoldemortLZFChunk _next;

    private VoldemortLZFChunk(byte[] data) {
        _data = data;
    }
     ...
     */
    public static VoldemortLZFChunk createCompressed(int origLen, byte[] encData, int encPtr, int encLen) {
        ...
        return new VoldemortLZFChunk(result);
    }

    /**
     * Factory method for constructing compressed chunk
     */
    public static VoldemortLZFChunk createNonCompressed(byte[] plainData, int ptr, int len) {
    ...
    public void setNext(VoldemortLZFChunk next) {
        _next = next;
    }

    public VoldemortLZFChunk next() {
        return _next;
    }
