 ...
 * 
 */
public class VoldemortChunkEncoder
{
     ...
     *        hash table to use
     */
    public VoldemortChunkEncoder(int totalLength) {
        int largestChunkLen = Math.max(totalLength, VoldemortLZFChunk.MAX_CHUNK_LEN);
     ...
     */
    public VoldemortLZFChunk encodeChunk(byte[] data, int offset, int len) {
        if(len >= MIN_BLOCK_TO_COMPRESS) {
            ...
            if(compLen < (len - 2)) { // nah; just return uncompressed
                return VoldemortLZFChunk.createCompressed(len, _encodeBuffer, 0, compLen);
            }
        }
        // Otherwise leave uncompressed:
        return VoldemortLZFChunk.createNonCompressed(data, offset, len);
    }
        ...
        while(inPos < inEnd) {
            ...
            if(ref < inPos && ref >= firstPos && (off = inPos - ref - 1) < MAX_OFF
               ...
               && in[ref] == (byte) (hash >> 16)) {
                ...
                out[outPos++] = in[inPos++];
                literals++;
                if(literals == VoldemortLZFChunk.MAX_LITERAL) {
                    out[outPos - literals - 1] = (byte) (literals - 1);
                    literals = 0;
                    outPos++;
                }
