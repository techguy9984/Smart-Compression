...
public class VoldemortLZFDecoder {
    ...
    public static byte[] decode(byte[] data) throws IOException {
        ...
        while(inPtr < (data.length - 1)) { // -1 to offset possible end marker
            ...
            inPtr += 2;
            if(type == VoldemortLZFChunk.BLOCK_TYPE_NON_COMPRESSED) { // uncompressed
    ...
    private static int calculateUncompressedSize(byte[] data) throws IOException {
        ...
        while(ptr < data.length) {
            ...
            try {
                if(data[ptr] != VoldemortLZFChunk.BYTE_Z || data[ptr + 1] != VoldemortLZFChunk.BYTE_V) {
                ...
                } else if(type == VoldemortLZFChunk.BLOCK_TYPE_COMPRESSED) { // compressed
    ...
    public static void decompressChunk(byte[] in, int inPos, byte[] out, int outPos, int outEnd)
        ...
        do {
            ...
            if(ctrl < VoldemortLZFChunk.MAX_LITERAL) { // literal run
                ctrl += inPos;
                do {
                    out[outPos++] = in[inPos];
                } while(inPos++ < ctrl);
